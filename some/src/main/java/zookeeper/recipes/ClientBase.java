...
public abstract class ClientBase extends ZKTestCase {
    ...
    static final File BASETEST =
        new File(System.getProperty("build.test.dir", "F:/"));

    //protected int port = PortAssignment.unique();
    protected int port = 2181;
    protected String hostPort = "172.20.183.137:" + port;
    ...
    protected ZooKeeper createClient(CountdownWatcher watcher)
        throws IOException, InterruptedException
    {
    ...
    public static void startServerInstance(File dataDir,
        ...
        LOG.info("STARTING server instance 172.20.183.137:{}", port);
        ZooKeeperServer zks = new ZooKeeperServer(dataDir, dataDir, 3000);
        factory.startup(zks);
        Assert.assertTrue("waiting for server up", ClientBase.waitForServerUp(
                "172.20.183.137:" + port, CONNECTION_TIMEOUT));
    }
    ...
    public static ServerCnxnFactory createNewServerInstance(
            ...
            throws IOException, InterruptedException {
        final int port = getPort(hostPort);
        LOG.info("CREATING server instance 172.20.183.137:{}", port);
        if (factory == null) {
